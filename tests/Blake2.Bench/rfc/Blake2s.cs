...
namespace Blake2Rfc
{
    static class Blake2s
    {
        unsafe public struct Blake2Context {
            public fixed byte b[64];                 // input buffer
            public fixed uint h[8];                  // chained state
            public fixed uint t[2];                  // total number of bytes
            public uint c;                           // pointer for b[]
            public uint outlen;                      // digest size
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static uint ror32(uint x, int y) => (x >> y) ^ (x << (32 - y));

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        unsafe static void mix(ref uint a, ref uint b, ref uint c, ref uint d, uint x, uint y)
        {
            a = a + b + x;
            d = ror32(d ^ a, 16);
            c = c + d;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        unsafe static uint getUInt32(byte* p) =>
            ((uint)p[0]         ) ^
            ((uint)p[1] << 1 * 8) ^
            ((uint)p[2] << 2 * 8) ^
            ((uint)p[3] << 3 * 8);
        ...
        static readonly byte[][] sigma = new byte[][] {
        ...
        unsafe static void compress(ref Blake2Context ctx, bool last)
        {
            ...
            for (uint i = 0; i < 16; i++)            // get little-endian words
                m[i] = getUInt32(&ctx.b[sizeof(uint) * i]);
        ...
        unsafe public static void final(ref Blake2Context ctx, void* hash)
        {
            ...
            for (uint i = 0; i < ctx.outlen; i++) {
                ((byte*)hash)[i] = (byte)((ctx.h[i >> 2] >> (int)(8 * (i & 3))) & 0xFF);
            }
        }
